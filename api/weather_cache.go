...
// WriteOneCall saves One Call API weather data to the cache file
func (cm *CacheManager) WriteOneCall(oneCall *OneCallResponse, todayData *TodayWeatherData) error {
	complete := logger.LogOperationStart("cache_write_onecall", map[string]any{
		"file_path": cm.filePath,
		"location":  todayData.Location,
	})

	// Get today's daily forecast (first element)
	if len(oneCall.Daily) == 0 {
		complete(fmt.Errorf("no daily data in One Call response"))
		return fmt.Errorf("no daily data to cache")
	}

	todayDaily := oneCall.Daily[0]

	// Create cache structure with proper daily min/max from One Call API
	now := time.Now()
	cache := WeatherCache{
		CreatedOn:     now.Format("2006-01-02"), // Local date
		CreatedAt:     now.Unix(),
	...
	complete(nil)
	logger.Debug("One Call weather cache saved: created=%s, location=%s, high=%.1f, low=%.1f",
		cache.CreatedOn, cache.Location, cache.DailyForecast.TempHigh, cache.DailyForecast.TempLow)
